...
export const MIN_PARTICIPANTS_ACTIVE = 3
export const MIN_PARTICIPANTS_CONFIRMED = 5
export const AUTO_CANCEL_HOURS = 6
export const REFUND_DEADLINE_HOURS = 72
...
export function getRefundAmount(eventDate, now = new Date(), price = 2) {
  const diffInHours = (eventDate - now) / (1000 * 60 * 60)
  if (diffInHours >= REFUND_DEADLINE_HOURS) return Math.floor(price / 2)
  return 0
}
...
export function getEventBadge(participantsCount, status, startAt) {
  ...
  if (participantsCount >= MIN_PARTICIPANTS_CONFIRMED) return { text: '✓ Confirmed', color: '#166534', bg: '#DCFCE7' }
  if (participantsCount >= MIN_PARTICIPANTS_ACTIVE) return { text: 'Small circle', color: '#92400E', bg: '#FEF3C7' }
...
export async function updateEventStatus(eventId) {
  ...
  if (newStatus !== event.status) {
    await updateDoc(meetupRef, { status: newStatus, updatedAt: serverTimestamp() })
    console.log(`📡 [updateEventStatus] Event ${eventId} status updated to ${newStatus}`)
  }
  return newStatus
}

// ─── FETCH – tous les événements (filtre status commenté pour le debug) ───
...
export async function fetchEventsByCity(city, maxResults = 20) {
  ...
  if (cityEvents.length >= maxResults) return cityEvents.slice(0, maxResults)
...
export async function fetchEventById(eventId) {
  ...
  if (meetupSnap.exists()) {
    ...
    return { id: meetupSnap.id, ...data, startAt: data.startAt || data.time || null }
  }
...
// ─── JOIN EVENT ─────────────────────────────────────────────────────────────
// (les fonctions suivantes sont identiques à ton code original, je les recopie)
export async function confirmPaidJoin(eventId, userId, userName, stripeSessionId, stripePaymentIntent) {
  const meetupRef = doc(db, 'meetups', eventId)
  try {
    const meetupSnap = await getDoc(meetupRef)
    if (!meetupSnap.exists()) throw new Error('EVENT_NOT_FOUND')
    const meetup = meetupSnap.data()
    if (meetup.status === EVENT_STATUS.CANCELLED) throw new Error('EVENT_CANCELLED')

    const meetupLimit = meetup.capacity_max || CAPACITY_MAX
    const currentParticipants = meetup.participants_count || 0
    if (currentParticipants >= meetupLimit) throw new Error('EVENT_FULL')
    ...
    if (!existingSnap.empty) throw new Error('ALREADY_JOINED')

    const newCount = currentParticipants + 1
    await updateDoc(meetupRef, { participants_count: newCount, updatedAt: serverTimestamp() })
...
export async function getUserEvents(userId, limitCount = 10) {
  ...
  const q = query(collection(db, 'meetup_participants'), where('user_id', '==', userId))
  const snap = await getDocs(q)
  ...
  for (const participant of sortedDocs) {
    ...
    if (eventSnap.exists()) {
      events.push({ ...participant, event: { id: eventSnap.id, ...eventSnap.data() } })
    }
  }
...
export async function markAttendance(eventId, userId, status) {
  if (!['attended', 'no_show'].includes(status)) throw new Error('Invalid status')
  const q = query(
    collection(db, 'meetup_participants'),
    where('event_id', '==', eventId),
    where('user_id', '==', userId)
...
export async function adminUpdateEventStatus(eventId, status) {
  await updateDoc(doc(db, 'meetups', eventId), { status, updatedAt: serverTimestamp() })
}
